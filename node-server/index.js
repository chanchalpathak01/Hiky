...
const transporter = nodemailer.createTransport({
  service: 'Gmail',
  auth: {
    user: 'chanchalnpathak@gmail.com',
    pass: 'xmgv fcte hyjr tfda',
  },
});
...
server.post('/recoversendOtp', async (req,res) => {
  try{
    ...
    else
    {
      ...
      const mailOptions = {
        from: 'chanchalnpathak@gmail.com',
        to: email,
...
server.get('/userBooking', async (req,res) => {
  ...
  try{
    ...
    for(var i=0;i<book_list.length;i++)
    {
      ...
      loc_details.push({Price : book_list[i].price, bookingId: book_list[i].bookingId, Place:book_list[i].locationName, Price:book_list[i].price, Coordinator_name: trip_list[0].tripCoordinatorName, ContactNo: trip_list[0].tripCoordinatorContactNumber, Date: trip_list[0].startDate});
